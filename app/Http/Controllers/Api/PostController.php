...
class PostController extends Controller
{
    ...
    public function listUserBookmark(Request $request)
    {
        ...
        if (isset($user) && !empty($user) && $user->delete_status == '0') {
            if (!empty($user)) {
                ...
                $url = env('APP_URL') . '/storage/app/public/';
                $bookmarks = PostBookmarkLike::with([
                    'post' => function ($q) use ($url) {
    ...
    public function postDetails(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            if (!empty($post)) {
                $post = Post::with([
                    'postcomments',
                    'postcomments.user' => function ($q) use ($uurl) {
                    ...
                    'postcomments' => function ($query) use ($user) {
                        ...
                        $query->with([
                            'commentlikes' => function ($qu) use ($user) {
                                if ($user) {
        ...
        if ($validator->passes()) {
            ...
            if (isset($user) && !empty($user) && $user->delete_status == '0') {
                ...
                $comment = PostComment::with([
                    'commentreplies',
                    'commentreplies.user' => function ($q) use ($url) {
    ...
    public function getDailyPostGuest(Request $request)
    {
        ...
        $posts->getCollection()->transform(function ($row) {

            $category = Postcategory::with([
                'category:id,name,slug'
            ])->where('post_id', $row->id)->first();

            if ($category) {
                $row->weburl = env('APP_URL') . '/' . $category->category->slug . '/' . $row->slug . '/detail';
            }
    ...
    public function getRecentPostGuest()
    {
        $date = Carbon::now()->subDays(5);
        $url = env('APP_URL') . '/storage/app/public/';
    ...
    public function getRelatedPostGuest(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'category_id' => 'required|exists:categories,id',
            'post_id' => 'required|exists:posts,id',
        ...
        $posts = DB::table('postcategories')
            ->join('posts', 'postcategories.post_id', '=', 'posts.id')
            ->select("posts.*", DB::raw("CONCAT('" . $url . "', posts.image) as image"))
            ->where('postcategories.category_id', $request->category_id)
        ...
        $posts->map(function ($row) {
            ...
            $row->total_post_comments = PostComment::where('post_id', $row->id)->count();
